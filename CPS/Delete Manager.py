...
class DeleteManager:
    def __init__(self, root):
        ...
        deleteFrame = Frame(bodyFrame, bg="#04c0cf").place(x=200, y=100, width=999, height=500);

        selectFloor = StringVar()
        selectFloor.set("Select FLOOR")
        selectFloorDropDown = OptionMenu(deleteFrame, selectFloor, "Ground Floor", "First Floor", "Second Floow", "Third Floor")
        selectFloorDropDown.place(x=520, y=200, width = 300, height = 50)

        selectManager = StringVar()
        selectManager.set("Select Manager ID")
        selectManagerDropDown = OptionMenu(deleteFrame, selectManager, "empID_1", "empID_2", "empID_3")
        selectManagerDropDown.place(x=520, y=300, width = 300, height = 50)

        deleteManagerBUutton = Button(deleteFrame, text="Remove Manager", cursor="hand2", bg="#097179", fg="white",
                               font=("times new roman", 20, "bold"), bd=4).place(x=550, y=400, width=250,
                                                                                 height=50)
        # =============================== FLOOR FRAME =============================================
